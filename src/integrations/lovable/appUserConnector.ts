 ...
 *
 * Nunca importar deste módulo em componentes/rotas do cliente: ele lê
 * segredos do servidor (`process.env`) e fala diretamente com o gateway
 * de conectores da Lovable.
 */
...
export interface CallAsAppUserParams {
  ...
  connectionAPIKey: string;
  connectorId: string;
  /** Caminho no provedor, ex.: "/calendar/v3/calendars/primary/events". */
  path: string;
  init?: RequestInit;
}
