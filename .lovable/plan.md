...
## Implementação
- Criar uma decisão pura e centralizada para o pós-ligação: resultado positivo (`SIM`/`ATENDEU`) nas etapas E0, E1, E2/V2, E3/V3, E4, E7, RE0, RE1 e RE3 resolve a chave existente `ENVIO_MATERIAL_POS_CONTATO`; resultado negativo mantém a chave e o contexto próprios da etapa.
...
- Ajustar RE0 e RE3 para ligação + mensagem imediata dentro da própria etapa, sem criar etapa, prazo, relógio ou fonte de verdade; preservar integralmente as transições existentes após a conclusão.
...
## Testes e validação
- Cobrir individualmente SIM e NÃO para E1, E2/V2, E3/V3, E4, E7, RE0, RE1 e RE3, além da manutenção da regra em E0.
- Confirmar que SIM sempre registra `ENVIO_MATERIAL_POS_CONTATO` e NÃO conserva a mensagem/contexto específico da etapa.
- Confirmar RE0 e RE3 no mesmo card, sem novo prazo, e ausência de alteração em `/s`, `/s/portal` e `/seg`.
